...
# prim <- readRDS(here::here("data/ctn0055/drv/randomization_and_induction.rds"))
# uds <- readRDS(here::here("data/ctn0055/drv/urine_opioids.rds"))
# tl <- readRDS(here::here("data/ctn0055/drv/timeline_followback_opioids.rds"))
prim <- readRDS("ctn_0094_0055_0067/data/ctn0055/drv/randomization_and_induction.rds")
uds <- readRDS("ctn_0094_0055_0067/data/ctn0055/drv/urine_opioids.rds")
tl <- readRDS("ctn_0094_0055_0067/data/ctn0055/drv/timeline_followback_opioids.rds")
...
urine <- select(prim, patid, prescribed_methadone, prescribed_bup) |>
  left_join(select(uds,
                   ...
                   urine_methadone,
                   urine_buprenorphine,
                   urine_opiates_2000_300_or_oxycodone),
            multiple = "all") |> #filter(patid == "0208500550022") %>% 
  mutate(urine_opioid = case_when(
    is.na(urine_date) ~ TRUE,
    urine_opiates_2000_300_or_oxycodone == 1 ~ TRUE,
    ...
    urine_methadone == 0 & urine_buprenorphine == 0 &
      urine_opiates_2000_300_or_oxycodone == 0 ~ FALSE
  )) |>
...
## find who missed a urine visit. If they miss a urine visit then we will need to see if they self reported. 
urine2 <- select(prim, randomization_date, patid, prescribed_methadone, prescribed_bup) |>
  left_join(select(uds,
                   patid,
                   visit,
                   urine_date,
...
# need to see if they drop out 
# if they are missing at least 1 urine visit
# need to check if they have self report in each 04 to 16 window. 
# if neither then they dropped out and the date of the window + 22 is recorded 
use <- use |> 
  group_by(patid, week) |>
...
use <- group_by(use, patid) |>
  ...
  select(patid, trt, induction, induction_date, randomization_date,
         ...
         selfreport_opioid_today, urine_opioid, use_today, days_of_use,
         was_observed, was_observed_obs_period, dropout_date,obs_period)
...
patid_4_week <- use %>%
  group_by(patid) %>%
  summarize(relapse_date_4week = if (any(!is.na(dropout_date))) min(dropout_date, na.rm = TRUE) else NA_Date_) %>%
  ungroup()

patid_4_week  %>% 
  filter(patid == "0208500550004") 


relapse_dates <- left_join(patid_7_day, patid_4_week) |>
  mutate(relapse_date = case_when(
    is.na(relapse_date_7day) & is.na(relapse_date_4week) ~ NA_Date_, # if both NA then NA
    is.na(relapse_date_7day) ~ relapse_date_4week,                   # if 7day NA then 4week
    is.na(relapse_date_4week) ~ relapse_date_7day,                   # if 4week is NA then 7day
...
# Save
saveRDS(relapse, "ctn_0094_0055_0067/data/ctn0055/drv/relapse_nov13.rds")
